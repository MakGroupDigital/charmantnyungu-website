...
const ClusteringViz = () => {
  ...
  return (
    <section className="py-24 max-w-7xl mx-auto px-6 grid md:grid-cols-2 gap-16 items-center">
      <div className="relative h-96 bg-white rounded-3xl border border-slate-200 shadow-inner overflow-hidden">
        {dots.map((dot, i) => (
          <div key={i} className={`absolute w-3 h-3 rounded-full ${dot.color} transition-all duration-1000 animate-pulse`} 
               style={{left: `${dot.x}%`, top: `${dot.y}%`}}></div>
        ))}
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
          <div className="w-1/2 h-1/2 border-2 border-dashed border-amber-200 rounded-full animate-spin-slow"></div>
        </div>
      </div>
      <div>
        <h2 className="text-4xl font-bold mb-6">Visualisation d'Algorithmes IA</h2>
        ...
        }} className="px-6 py-3 bg-slate-900 text-white rounded-xl font-bold hover:bg-amber-600 transition-colors">Relancer la simulation</button>
      </div>
    </section>
  );
};
  ...
  return (
    <section className="py-24 bg-white">
      <div className="max-w-4xl mx-auto px-6 text-center">
        <h2 className="text-4xl font-bold mb-4">Quiz : Tech Africaine</h2>
        ...
        {!showResult ? (
          <div className="bg-slate-50 p-8 rounded-3xl border border-slate-200">
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              {questions[current].options.map(opt => (
                <button key={opt} onClick={() => handleAnswer(opt)} className="p-4 bg-white border border-slate-200 rounded-xl hover:border-amber-500 hover:text-amber-600 transition-all font-bold">
                  {opt}
                </button>
              ))}
            </div>
          ...
          <div className="bg-amber-50 p-12 rounded-3xl border border-amber-200">
            ...
            <button onClick={() => {setCurrent(0); setScore(0); setShowResult(false);}} className="bg-amber-600 text-white px-8 py-3 rounded-xl font-bold">Recommencer</button>
          </div>
        )}
      </div>
    </section>
...
const ROICalculator = () => {
  ...
  return (
    <section className="py-24 max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-16 items-center">
      <div>
        <h2 className="text-4xl font-bold mb-6">Calculateur de ROI Technologique</h2>
        <p className="text-slate-600 leading-relaxed mb-8">
        ...
        <div className="grid gap-4">
           {Object.keys(inputs).map(key => (
             <div key={key}>
               ...
               <input type="number" value={inputs[key as keyof typeof inputs]} onChange={e => setInputs({...inputs, [key]: Number(e.target.value)})} className="w-full bg-white border border-slate-200 p-3 rounded-lg focus:outline-none focus:border-amber-500" />
             </div>
           ))}
        </div>
      </div>
      <div className="bg-slate-900 text-white p-12 rounded-3xl text-center shadow-2xl">
...
const PhishingGame = () => {
  ...
  return (
    <section className="py-24 bg-white">
      <div className="max-w-4xl mx-auto px-6 text-center">
        <h2 className="text-4xl font-bold mb-4">Détecteur de Phishing</h2>
        ...
        <div className="bg-slate-50 p-8 rounded-3xl border border-slate-200 text-left relative overflow-hidden">
           ...
           {feedback && <div className="absolute inset-0 bg-slate-900/90 flex items-center justify-center text-white text-2xl font-bold animate-in fade-in">{feedback}</div>}
        </div>
      </div>
    </section>
  );
